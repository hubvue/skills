# 更新日志

本项目的所有重要更改都将记录在此文件中。

## 1.17.0 - 2026-09-01

### 功能特性
- `minimal`: 新增 productivity 技能，在代码实现、缺陷修复、重构、UI 或数据逻辑改动、测试与代码审查中强制最小正确改动。增加条件、兜底、兼容逻辑、抽象、文件、状态或依赖前必须有依据，随后从 diff 中减去无依据复杂度，并在验收条件满足后立即停止。

### 文档
- 更新 `README.md` 与 `README.zh.md`，补充 `minimal` 说明及 marketplace 注册。

## 1.16.0 - 2026-08-19

### 功能特性
- `decision-boundary`: 新增 productivity 技能，在交付导向的软件工作中作为后台护栏被动约束 LLM 行为。区分已确认事实与假设、保留用户决策权、选择最小充分设计、只做可追溯的外科手术式改动、用证据消除未知、核验既定目标并在完成后停止。用户明确要求头脑风暴、探索或多方案前瞻架构时，不压制广度。

### 文档
- 更新 `README.md` 与 `README.zh.md`，补充 `decision-boundary` 说明及 marketplace 注册。

## 1.15.0 - 2026-08-12

### 功能特性
- `tech-writing`: 新增 productivity 技能，将复杂工程项目、框架、工具与实践转化为以读者为中心的中英文技术文章。支持仓库调研、文章定位、逐节共写、证据支撑示例、视觉规划、事实核验，以及对 README 风格或 AI 味过重内容的改写。

### 文档
- 更新 `README.md` 与 `README.zh.md`，补充 `tech-writing` 说明及 marketplace 注册。

## 1.14.0 - 2026-08-04

### 功能特性
- `idea-incubation`: 用单一 productivity 技能替代原 `idea-requirement` 插件，通过 12 个内部阶段将原始想法孵化为可决策需求。接口为 `/idea-incubation <phase> [阶段输入]`。新增输入/输出质量门禁、可持久化单问题上下文循环、确定性路由/重流/幂等、工作区校验与中断恢复。
- `breaking-refactor`: 新增 project 技能，用于规划、执行并验证有意为之的破坏性重构——在不保留运行时向后兼容的前提下替换既有设计，覆盖影响分析、切换计划、消费方同步更新，以及旧设计已清除的证据。
- `project-migration`: 大幅增强，补充循环规则、更丰富的产物模板、迁移检查清单、扩展阶段指南、风险手册与状态 schema，以支撑更安全的分阶段迁移。

### 重构
- 移除 `idea-requirement` 插件及其 12 个公共节点技能（`idea-intake`、`motivation-clarify`、`painpoint-validate`、`scenario-restore`、`user-identify`、`goal-define`、`value-assess`、`scope-define`、`feasibility-assess`、`solution-design`、`acceptance-criteria`、`requirement-assemble`）。调用方需迁移到 `/idea-incubation <phase>`。

### 问题修复
- `idea-incubation`: 补齐阶段状态与决策映射、增加 `blocked/retry` 恢复、移除公开工作流参数、统一证据等级与来源新鲜度，并修复 assemble 阶段对自身研究文档的循环依赖。

### 其他
- 新增 `skills-site` 子模块，用于技能文档/站点。

### 文档
- 更新 `README.md` 与 `README.zh.md`，补充 `idea-incubation`、`breaking-refactor` 及相关 marketplace 注册信息。

## 1.13.0 - 2026-06-23

### 功能特性
- `idea-requirement`: 新增插件分类，提供以证据驱动、将原始想法转化为完整需求的工作流。包含 `idea-requirement` 编排器以及 12 个阶段技能（`idea-intake`、`motivation-clarify`、`painpoint-validate`、`scenario-restore`、`user-identify`、`goal-define`、`value-assess`、`scope-define`、`feasibility-assess`、`solution-design`、`acceptance-criteria`、`requirement-assemble`）。每个阶段都会持久化带编号的阶段文档，并通过 Web Search 证据支撑决策。
- `clean-code`: 新增 project 技能，提供 Clean Code 规范总览与路由，覆盖命名、函数、注释、格式、对象与数据结构、错误处理、边界、测试、类、系统、并发、简单设计、渐进式重构及各类代码坏味道，并按需加载 references 中对应的规则文件。

### 文档
- 更新 `README.md` 与 `README.zh.md`，补充 `idea-requirement` 分类与 `clean-code` 技能说明。

## 1.12.0 - 2026-04-14

### 功能特性
- `code-evolver`: 新增 context-engineering 技能，将对话中反复出现的代码规范、工程约定、架构约束与协作规则沉淀为带 few-shot 的可维护项目规则。识别新增/更新/合并/冲突，过滤一次性指令，并按职责同步到 `.cursor/rules`、`AGENTS.md` 与 `CLAUDE.md`。

### 文档
- 更新 `README.md` 与 `README.zh.md`，补充 `code-evolver` 说明。

## 1.11.0 - 2026-04-08

### 功能特性
- `doc-skill-creator`: 新增将现有文档转换为专用技能设计的技能。将组件库文档、SDK 文档、工具文档、FAQ、迁移指南、标准文档和示例等静态文档转换为实用的技能工件。分析适用性、提取能力、设计技能结构，并起草 SKILL.md 和参考资料。

## 1.10.0 - 2026-03-26

### 功能特性
- `project-migration`: 新增分阶段推进仓库迁移和工程迁移的技能。支持 intake、audit、map、diff、plan、pilot、execute、verify、cleanup，并提供可复用的迁移模板、检查清单和风险指引。

### 文档
- 更新 `README.md` 和 `README.zh.md`，补充 `project-migration`，并修正 project 分类下 `project-handover-generator` 与 `doc-generator` 的说明。

## 1.9.0 - 2026-03-25

### 功能特性
- `doc-generator`: 新增基于源代码生成、更新或改进项目、文件、组件、SDK、模块或函数的使用文档的技能。支持项目级别、文件级别和符号级别目标。生成使用导向的文档，包括何时/如何使用、约束和常见陷阱。

## 1.8.2 - 2026-03-24

### 重构
- `dev-workflow` → `dev-spec`: 重命名技能以提高清晰度。

## 1.8.1 - 2026-03-16

### 文档
- `deep-learning`: 对技能文档进行细微优化。

## 1.8.0 - 2026-03-16

### 功能特性
- `deep-learning`: 新增系统化学习和解释库、框架、模块、函数或代码路径原理的技能。帮助用户构建正确的思维模型，提供从问题和角色到设计权衡和下一步阅读的分层解释。支持项目、模块和函数三种模式，提供全面的工作流指导。

### 其他
- 升级 gitignore，添加更多模式。

## 1.7.1 - 2026-03-10

### 功能特性
- `dev-workflow`: 添加测试和修复阶段，完善开发生命周期覆盖。新增工件规则文档和状态模式，改进结构验证。

## 1.7.0 - 2026-03-06

### 功能特性
- `dev-workflow`: 新增文档驱动的 AI 代理开发工作流技能，通过任务编排，包含明确阶段（intake、research、plan、todo、implement、review）、每个任务的工件、依赖修复和迭代需求的增量更新。
- `project-handover-generator`: 新增项目交接文档生成技能，从代码库和工学生成实用的项目交接文档，基于实际仓库证据建立内容。

## 1.6.2 - 2026-01-26

### 重构
- `context-probe`: 简化了 SKILL.md 文档（从 250 行减少到 73 行），提高了清晰度和可维护性。

## 1.6.1 - 2026-01-26

### 重构
- `context-probe`: 将技能从隐式监控器重构为安装程序 + 防护栏规则，支持斜杠命令 UX。添加 v1 版本管理的托管块格式，支持 AGENTS/Cursor/Claude 优先级层，以及去重/迁移功能。

### 文档
- 更新 AGENTS.md，添加全面的 context-probe 安装命令和托管块文档。
- 更新 CLAUDE.md，添加 context-probe 安装说明。

## 1.6.0 - 2026-01-26

### 功能特性
- `context-probe`: 新增隐式系统级技能，监控上下文完整性、系统提示保留情况和指令漂移。始终隐式运行，无需用户调用。当上下文降级或丢失时发出警告。

### 文档
- 更新 CLAUDE.md，添加 context-probe 文档。
- 更新 README，添加 context-probe 技能描述。

## 1.5.1 - 2026-01-26

### 功能特性
- `prompt-minifier`: 新增 prompt_only 输出模式支持。用户现在可以在 `prompt_only`（仅压缩提示词）和 `prompt_with_report`（压缩提示词 + 压缩报告）之间选择。

### 文档
- 更新 CLAUDE.md，整合了插件组织和代码风格指南。

## 1.5.0 - 2026-01-26

### 功能特性
- `skills-workflow-builder`: 新增工作流技能创建器，通过嵌入固定管道（技能顺序 + 每步目标/模板/合同/停止条件）创建专用工作流技能。复用 skills-workflow 进行规范化，复用 skill-creator 进行实现。

### 文档
- 更新 README，添加 skills-workflow-builder 文档。

## 1.4.0 - 2026-01-26

### 功能特性
- 将 `skills-orchestrator` 重命名为 `skills-workflow`，以提高清晰度和命名一致性。

## 1.3.0 - 2026-01-26

### 功能特性
- `prompt-minifier`: 新增提示词最小化技能，可在保持行为语义等价的前提下，将冗长的提示词压缩为精简格式。支持无损、平衡和激进三种压缩级别。
- `context-engineering`: 新增上下文工程插件分类。
- 将 `prompt-interviewer` 从 productivity 插件迁移到 context-engineering 插件。

### 文档
- 更新 README，添加上下文工程分类和提示词最小化技能文档。

## 1.2.0 - 2026-01-23

### 功能特性
- `skills-orchestrator`: 新增交互式技能编排技能。支持将多个技能串联，使步骤 i 的输出成为步骤 i-1 的输入。支持 dry-run 模式、跟踪日志、输出验证以及安全防护，包括提示注入保护和密钥脱敏。

### 文档
- 更新 README，添加 skills-orchestrator 文档

## 1.1.0 - 2026-01-22

### 功能特性
- `api-generator`: 新增自动前端 API 代码生成技能，支持项目感知集成。可检测 TypeScript、请求模式、mock 基础设施和测试框架，生成受功能限制的代码。
- `prd-gatekeeper`: 工程 PRD 看门人，通过 5 个严格质量门槛验证 PRD，确保为工程师提供生产就绪的文档。

### 文档
- 更新 README，添加新技能文档
- 添加 CHANGELOG.md 和 CHANGELOG.zh.md 用于版本跟踪

## 1.0.1 - 2026-01-22

### 文档
- 更新 README，改进格式和结构

## 1.0.0 - 2026-01-22

### 初始发布

kim-skills Claude Code 插件的首次正式发布。

### 功能特性

**前端分析 (fe-analysis)**
- `architecture-analysis` - 综合前端架构分析器，可识别技术栈、构建工具和架构模式。支持检测 Vue/React/Angular 框架、Node.js 环境、包管理器、TypeScript 使用情况、代码检查工具和架构模式。提供多种输出格式，包括 JSON、Markdown、执行摘要和评分卡。

- `dependency-analysis` - 增强的依赖分析器，提供全面的 Markdown 报告和可操作的建议。执行安全扫描、未使用包检测、幽灵依赖识别、循环依赖检测和清理脚本生成。支持 JavaScript、TypeScript、Vue、React、Angular 和现代构建工具，具有并行处理和增量分析功能。

- `unit-test-generator` - 智能单元测试生成器，可检测现有测试框架并为函数、组件和模块生成综合测试。通过分析现有测试模式保持框架一致性。支持 Jest、Vitest、Mocha 以及 React、Vue、Angular 框架。

**项目 (project)**
- `resume-project-analyzer` - 通过 5 步工作流程将代码库转换为真实、可防御的简历项目经验。分析项目结构、提取工程价值、对信心级别进行分类、进行反思性提问并生成结构化输出。

- `prd-gatekeeper` - 工程 PRD 看门人，通过 5 个严格质量门槛验证 PRD，确保为工程师提供生产就绪的文档。当存在工程风险时阻止进度，确保没有隐含的产品决策被推送到实现阶段。

**生产力 (productivity)**
- `prompt-interviewer` - 高级提示工程师和提示面试官，通过结构化分析和迭代式提问来帮助用户完善和完成提示。作为面试官而不是提示重写器，确保提示在执行前明确且结构良好。

- `release-skills` - 自动化 kim-skills 插件的发布流程：分析自上次标签以来的更改、更新 CHANGELOG（中英文）、更新 marketplace.json 版本、提交并创建版本标签。
